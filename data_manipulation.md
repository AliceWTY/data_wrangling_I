...
We want to see litter_number, but still keep everthing else.
litter_number becomes the first variable in the table.
...
two “=” sign, “==”, means testing the equality For gf_of_birth that are
equal to 20.
...
Or we use a more general way to get gf_of_birth that are NOT equal to
20. Asking for the samples that not fill in given condition. “!()”
...
Arrange by two variables. Arrange the data by pups_born_alive. Then,
under that arrange data by gd0_weight.
...
By useing pipe, we don’t need to specify data frames. By conecting two
command by ” %\>% “, we are telling R to feed the dataset get from the
previous command directly to the following command.
